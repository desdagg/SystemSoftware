...
int readFile(int client_sockfd, char** userName, char **tmpFileName){
  ...
  *tmpFileName = malloc(100);
  ...
  printf("[server] Temp Receive File name is == %s\n", *tmpFileName);
  ...
  if(fr == NULL)
    ...
    //receive input buffer from client in blocks of MAX_BUF size 
...
int copyFileToSite(char **tmpFileName,char **targetFileName, char **dirCode, char **destPath){
  ...
  *destPath = malloc(250);
  ...
  printf("[server] Destination File path %s/n", targetFolder);
  strcpy(cmd, "cp ");
